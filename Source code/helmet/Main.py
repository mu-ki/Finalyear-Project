...
def main(imgOriginalScene):
    ...
    if len(listOfPossiblePlates) == 0:                          # if no plates were found
        ...
        if len(licPlate.strChars) == len(NumberLength):
            
            filewrite = open('result.txt','w')
            
            filewrite.writelines(licPlate.strChars)
    ...
    cv2.waitKey(10)					# hold windows open until user presses a key
...
if face_cascade is None:
    print('error:read cascade')
    os.system('pause')
    
cap = cv2.VideoCapture('kajal.mp4')
if cap is None:
    print('error: in read a video')
    os.system('pause')    
while True:
    ...
    width,height = 800,500  #frame size
    
    frameresize = cv2.resize(frame,(width,height)) #resize the frame
    ...
    #crop an image
        ...
        crop = frameresize[y:y+h,x-40:x+w]  #crop an images
